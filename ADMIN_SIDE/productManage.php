    ...
    <section class="products container section--lg">
      ...
      <ul class="pagination">
        <?php if ($currentPage > 1): ?>
          <li><a href="?page=<?= $currentPage - 1 ?>&search=<?= urlencode($search) ?>&filter=<?= urlencode($filter) ?>" class="pagination__link">«</a></li>
        <?php else: ?>
        ...
        <?php
        if ($currentPage > 3) {
            echo '<li><a href="?page=1&search=' . urlencode($search) . '&filter=' . urlencode($filter) . '" class="pagination__link">1</a></li>';
            if ($currentPage > 4) echo '<li class="pagination__dots">...</li>';
        }

        for ($i = max(1, $currentPage - 2); $i <= min($totalPages, $currentPage + 2); $i++) {
            echo '<li><a href="?page=' . $i . '&search=' . urlencode($search) . '&filter=' . urlencode($filter) . '" class="pagination__link ' . ($i == $currentPage ? 'active' : '') . '">' . $i . '</a></li>';
        }

        if ($currentPage < $totalPages - 2) {
            if ($currentPage < $totalPages - 3) echo '<li class="pagination__dots">...</li>';
            echo '<li><a href="?page=' . $totalPages . '&search=' . urlencode($search) . '&filter=' . urlencode($filter) . '" class="pagination__link">' . $totalPages . '</a></li>';
        }
        ?>
